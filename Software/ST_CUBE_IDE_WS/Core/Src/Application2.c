...
void StateMachine(void){
	...
	//
	if(TIM2->CNT<0x8009){//if encoder less than 0x7FFF+0x000A=0x8009 - 100ï¿½C
		TIM2->CNT=0x8009;//low level saturation at 0x8009
	}
	if(TIM2->CNT>0x802C){//if encoder bigger than 0x7FFF+0x002D=0x802C - 450ï¿½C
	...
	uint8_t i = 0;
	TmpBuf[i] = (temp / 100) + 0x30;		//szÃ¡zas
	if (TmpBuf[i] != '0') {
		i++;
	}
	temp %= 100;
	...
	TmpBuf[i++] = ' ';
	TmpBuf[i++] = '\0';
	LCD_write(0x8A, 0x00);		//LCD elsÅ‘ sor
